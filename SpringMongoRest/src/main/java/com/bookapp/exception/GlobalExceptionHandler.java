...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMissingPathVariable(MissingPathVariableException ex, HttpHeaders headers,
        ...
        LocalDateTime timeStamp  = LocalDateTime.now();
        List<String> details =
                Arrays.asList("missing path variable","No variables");
        APIErrors errors =
          new APIErrors(message, timeStamp, status, details);
        return ResponseEntity.status(status).body(errors);
	}
	...
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
        ...
        LocalDateTime timeStamp  = LocalDateTime.now();
        List<String> details =
                Arrays.asList("mismatch of data","Wrong Type");
        APIErrors errors =
          new APIErrors(message, timeStamp, status, details);
        return ResponseEntity.status(status).body(errors);
	}
	...
	public ResponseEntity<Object> handleBookException(Exception ex){
		String message = ex.getMessage();
		LocalDateTime timeStamp = LocalDateTime.now();
		List<String> details=Arrays.asList("Book is not available");
